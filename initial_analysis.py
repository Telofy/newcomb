...
def summary_statistics(feature_names, excluded_studies=(21,)):
    ...
    dataframes_for_each_study = utils.split_dataset_by_study(
        data, feature_names, excluded_study_labels=excluded_studies
    )
...
def leave_one_study_out_analysis(
    feature_names=None,
    ...
    for test_study_number, X_and_y_test in dataframes_for_each_study.items():
        ...
        for study_number, X_and_y_ in dataframes_for_each_study.items():
            if study_number != test_study_number:
                ...
                X_, y_ = X_and_y_
                if np.array_equal(
                    np.intersect1d(X_.columns, feature_names), np.sort(feature_names)
                ):
